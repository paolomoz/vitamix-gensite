...
const analyticsTracker = getAnalyticsTracker({
  endpoint: 'https://vitamix-gensite-analytics.paolo-moz.workers.dev',
});
analyticsTracker.init();
window.analyticsTracker = analyticsTracker;

// Worker URLs
const GENERATIVE_WORKER_URL = 'https://vitamix-gensite-recommender.paolo-moz.workers.dev';
const FAST_WORKER_URL = 'https://vitamix-gensite-recommender.paolo-moz.workers.dev';
...
async function renderGenerativePage() {
  ...
  eventSource.addEventListener('block-content', async (e) => {
    ...
    if (blockEl) {
      ...
      // For heroes, check if aspect-wide and propagate to wrapper/section immediately
      if (blockName === 'hero' && blockEl.classList.contains('aspect-wide')) {
...
async function renderVitamixRecommenderPage() {
  ...
  let eventSource;
  let useExtPrefetch = false;
  if (extPrefetch && extPrefetch.events && extPrefetch.events.length > 0) {
    // Extension already generated content via background service worker
    // Use a dummy EventTarget to replay buffered events — no new SSE needed
    useExtPrefetch = true;
    eventSource = new EventTarget();
    eventSource.close = () => {}; // no-op for compatibility
    // Update slug to match the prefetched generation
    if (extPrefetch.slug) {
      // eslint-disable-next-line no-param-reassign
      slug = extPrefetch.slug;
    }
    // eslint-disable-next-line no-console
    console.log(`[Recommender] Using extension prefetch: ${extPrefetch.events.length} events, ${extPrefetch.headStart}ms head start`);
  } else if (prefetch) {
